...
class MarketAnalysisPanel(QWidget):
    ...
    def _create_toolbar(self):
        ...
        self.symbol_combo.addItems(["AAPL", "MSFT", "GOOGL"])
        self.symbol_combo.currentTextChanged.connect(self._on_symbol_change)
        self.toolbar_layout.addWidget(self.symbol_combo)
    ...
    def _load_market_data(self):
        """加载市场数据"""
        # 检查市场到量子转换器是否已启动
        if not self._check_component('converter'):
            return
    ...
    def _analyze_market_data(self):
        """分析市场数据"""
        # 检查组件是否已启动
        if not self._check_component('analyzer') or not self._check_component('converter') or not self._check_component('interpreter'):
            return
            
        if not self.market_data or not self.current_symbol:
    ...
    def _create_sample_data(self):
        ...
        for symbol in symbols:
            ...
            highs = [close * np.random.uniform(1.0, 1.03) for close in closes]
            lows = [close * np.random.uniform(0.97, 1.0) for close in closes]
            volumes = [int(np.random.uniform(100000, 1000000)) for _ in closes]
