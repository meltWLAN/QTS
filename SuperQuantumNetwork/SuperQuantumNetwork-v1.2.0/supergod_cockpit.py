...
class RecommendedStocksPanel(QFrame):
    ...
    def load_recommended_stocks(self):
        ...
        try:
            ...
            for row, stock in enumerate(stocks):
                ...
                # 根据涨跌幅设置颜色
                if change_pct > 0:
                    change_item.setForeground(QBrush(QColor(SupergodColors.POSITIVE)))
                elif change_pct < 0:
                    change_item.setForeground(QBrush(QColor(SupergodColors.NEGATIVE)))
    ...
    def get_recommended_stocks(self):
        """获取推荐股票列表"""
        # 检查父窗口是否有数据连接器
        if hasattr(self.parent_cockpit, 'data_connector') and self.parent_cockpit.data_connector:
            try:
                # 尝试使用数据连接器获取推荐股票
                if hasattr(self.parent_cockpit.data_connector, 'get_recommended_stocks'):
                    self.logger.info("尝试使用数据连接器获取推荐股票...")
                    stocks = self.parent_cockpit.data_connector.get_recommended_stocks()
                    if stocks and len(stocks) > 0:
                        self.logger.info(f"成功获取 {len(stocks)} 只推荐股票")
                        return stocks
                    
                # 如果没有get_recommended_stocks方法，尝试直接使用Tushare获取实时数据
                self.logger.info("尝试直接使用Tushare VIP获取实时股票数据...")
                return self.get_real_stock_recommendations()
                    
            except Exception as e:
                self.logger.warning(f"通过数据连接器获取推荐股票失败: {str(e)}")
                self.logger.info("尝试直接使用Tushare VIP获取实时股票数据...")
                try:
                    return self.get_real_stock_recommendations()
                except Exception as e2:
                    self.logger.error(f"获取实时股票数据失败: {str(e2)}")
        
        # 如果前面的都失败了，才使用模拟数据
        self.logger.warning("无法获取真实数据，使用模拟推荐数据")
...
def main():
    ...
    palette.setColor(QPalette.HighlightedText, QColor(SupergodColors.TEXT_PRIMARY))
