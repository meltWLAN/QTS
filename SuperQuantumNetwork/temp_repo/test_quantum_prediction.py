...
def main():
    ...
    try:
        ...
        # 设置测试股票
        test_stock_code = "600519.SH"  # 贵州茅台
        test_stock_name = "贵州茅台"
        test_price = 1800.0  # 假设当前价格
        ...
        if prediction_result and prediction_result.get('success', False):
            ...
            # 将结果保存到文件
            with open('quantum_prediction_result.json', 'w', encoding='utf-8') as f:
                json.dump(prediction_result, f, ensure_ascii=False, indent=2)
            logger.info("预测结果已保存到 quantum_prediction_result.json")
        else:
            logger.error("预测失败！")
