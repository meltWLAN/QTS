...
class TushareDataSource:
    ...
    def get_market_status(self):
        ...
        Returns:
            dict: 市场状态信息
        """
        if not self._ensure_api_ready() and not self.offline_mode:
            self.logger.error("TuShare API未准备好，无法获取市场状态")
            return None
        ...
        def fetch_func():
            ...
            try:
                ...
                # 获取最近交易日
                last_df = self.api.trade_cal(exchange='SSE', start_date=(datetime.date.today() - datetime.timedelta(days=10)).strftime("%Y%m%d"), end_date=today)
                last_df = last_df[last_df['is_open'] == 1]
                last_trading_day = last_df['cal_date'].values[-2] if len(last_df) > 1 else today
                
                # 获取下一个交易日
                next_df = self.api.trade_cal(exchange='SSE', start_date=today, end_date=(datetime.date.today() + datetime.timedelta(days=10)).strftime("%Y%m%d"))
                ...
                # 判断当前是否在交易时间内
                trading_am = datetime.time(9, 30) <= now.time() <= datetime.time(11, 30)
                trading_pm = datetime.time(13, 0) <= now.time() <= datetime.time(15, 0)
                
                # 确定市场状态
                status = "休市"
                if is_open:
                    if trading_am or trading_pm:
                        status = "交易中"
                    elif now.time() < datetime.time(9, 30):
                        status = "开盘前"
                    elif datetime.time(11, 30) < now.time() < datetime.time(13, 0):
                        status = "午休"
        ...
        # 使用缓存获取数据
        return self._get_data_with_cache("market_status", fetch_func, force_refresh=True, cache_duration=300)  # 5分钟缓存
